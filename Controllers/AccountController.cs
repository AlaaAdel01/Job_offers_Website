...
namespace WebApplication4.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            var userId =User.Identity.GetUserId();
            var curUser = db.Users.Where(a => a.Id == userId).SingleOrDefault();
            if(!UserManager.CheckPassword(curUser,profile.Password))
            {
                ViewBag.message = "current password wrong";
            }
            else
            {
                var newHasPassword = UserManager.PasswordHasher.HashPassword(profile.NewPassword);
                curUser.UserName = profile.UserName;
                curUser.Email = profile.Email;
                curUser.PasswordHash = newHasPassword;
                db.Entry(curUser).State = System.Data.Entity.EntityState.Modified;
                db.SaveChanges();
